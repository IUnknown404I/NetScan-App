...
def run_nmap_scan_list(ip_address):
    ...
    f_stop_time = sec
    print('\n[*] Process for {ip_address} done! Elapsed time: {elapsed_time}'.format(ip_address=ip_address, elapsed_time=f_stop_time))
    nettemp.write('\n[*] Process for {ip_address} done! Elapsed time: {elapsed_time}\n\n'.format(ip_address=ip_address, elapsed_time=f_stop_time))
    nettemp.close()
...
def run_nmap_scan(ip_address):
    ...
    print('\n[***] NMAP process for {ip_address} done! Elapsed time: {elapsed_time}'.format(
        ip_address=ip_address, elapsed_time=stop_time)
    )
    return host_info
...
def DevT_byPorts():
        ...
        if ((lports.__contains__(80) or lports.__contains__(443) or lports.__contains__(8080)) and server_perc!=100 and router_perc!=100):
            if not lports.__contains__(135) and not lports.__contains__(139) and not lports.__contains__(5357):
                ...
                for port in lports:
                    ...
                    elif port == 22:   server_perc=server_perc+3
                    elif port == 23:   server_perc=server_perc+3
    ...
    if len(signatures)!=0: print('signature ports:     {l}'.format(l=signatures))
    if len(connect)!=0: print('connect&fs ports:    {l}'.format(l=connect))
    if len(web)!=0: print('web ports:           {l}'.format(l=web))
    if len(private)!=0: print('private-using ports: {l}'.format(l=private))
    if len(others)!=0: print('service ports:       {l}'.format(l=others))
...
try:
    ...
    print('Type ipv4 of network (x.x.x.x/24) or host. to start analyzing.')
