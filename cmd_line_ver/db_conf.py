                ...
                if len_check:
                    ...
                    if temp_size!=host_size:
                        if temp_size>host_size: 
                            ...
                            for ev_line in hostout:
                                if tempout.count(ev_line)==0: err_list.append('Line {number}:   '.format(number = hostout.index(ev_line)+1)+ev_line.strip())
                    else:
                        if temp_size>host_size: 
                            for eq_line in tempout:
                                if hostout.count(eq_line)==0: err_list.append('Line {number}:   '.format(number = tempout.index(eq_line)+1)+eq_line.strip())
